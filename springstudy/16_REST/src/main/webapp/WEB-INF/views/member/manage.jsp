...
<script>
	...
	function fnRemove(){
		$('#btnRemove').on('click', function(){
			...
			for(let i = 0; i < $('.checkOne').length; i++){
				// console.log($('.checkOne')[i].value)		// 자바스크립트이기 때문에 .value
				// console.log( $($('.checkOne')[i]).val() );	// .val() 메소드를 사용하려면 jquery로 사용해야하기 때문에 $()로 전체 다 묶어줘야한다.
	...
	function fnPagingLink(){
		$(document).on('click', 'enable_link', function(){
			page = $(this).data('page');	// 호이스팅 때문에 아래 var page = 1; 부터 실행
			fnList();
		})
	}
	...
	// 4. 회원목록 + page 전역변수
	var page = 1;	// 초기화 최초 1번 사용
	function fnList(){
		$.ajax({
			url: '${contextPath}/members/page/' + page,
			type: 'GET',
			dataType: 'json',				// url에 데이터가 포함되어 있어 data 항목은 없다.
			success: function(obj){
	...
	function fnAdd(){
		$('#btnAdd').on('click', function(){
			...
			// 자바스크립트 객체를 stringify를 통해 문자열객체로 만든다. 
			// DB로 보낼 때 Jackson이 개입해서 DTO나 Map으로 변환.
			let member = JSON.stringify(   // 문자열로 만드는 함수 stringify()
				{
					id: $('#id').val(),
					name: $('#name').val(),
					gender: $(':radio[name="gender"]:checked').val(),    // checked 된 gender
					address: $('#address').val()
				}		
			);
			// ajax 처리
	...
	function fnInit(){
		...
		$('#name').val('');
		$(':radio[name="gender"]').prop('checked', false);   // gender는 radio라서 value를 없애면 안되고 체크를 해제해야 한다.
															 // 콜론(:)은 input의 type 부르는 방법. 풀네임 : 'input:radio[name="gender"]'
		$('#address').val('');
	}
